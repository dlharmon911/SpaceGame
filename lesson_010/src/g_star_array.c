#include <allegro5/allegro5.h>
#include <stdint.h>
#include <string.h>
#include <libspace.h>
#include "g_constants.h"
#include "g_texture_data.h"
#include "g_model_data.h"
#include "g_star.h"
#include "g_star_array.h"

enum
{
	G_STAR_SCALE_ARRAY_SIZE = 5
};

void g_star_array_set_zero(g_star_array_t* star_array)
{
	if (!star_array)
	{
		return;
	}

	for (size_t i = 0; i < G_STAR_COUNT; ++i)
	{
		g_star_set_zero(&star_array->m_star[i]);
	}
	star_array->m_count = G_STAR_COUNT;
}

void g_star_array_draw(const g_star_array_t* star_array, int32_t flag)
{
	if (!star_array)
	{
		return;
	}

	for (size_t i = 0; i < G_STAR_COUNT; ++i)
	{
		g_star_draw(&star_array->m_star[i], flag);
	}
}

void g_star_array_scatter(const g_texture_data_t* texture_list, g_star_array_t* star_array)
{
	if (!texture_list)
	{
		return;
	}

	if (!star_array)
	{
		return;
	}

	const uint32_t scale[G_STAR_SCALE_ARRAY_SIZE] =
	{
		G_STAR_LAYER_01,
		G_STAR_LAYER_02,
		G_STAR_LAYER_03,
		G_STAR_LAYER_04,
		G_STAR_LAYER_05
	};
	size_t i = 0;
	uint32_t lower = 0;
	uint32_t upper = 0;
	g_star_t* star = NULL;

	for (size_t j = 0; j < G_STAR_SCALE_ARRAY_SIZE; ++j)
	{
		upper += scale[j];

		for (size_t k = 0; k < scale[j]; ++k, ++i)
		{
			float a = (float)s_random_generate_constraint(lower, upper - 1);

			star = star_array->m_star + i;
			star->m_z = s_math_min_f(1.0f, a / (float)G_STAR_COUNT) * 0.5f;

			g_model_generate_star_model(&star->m_star_model, &star->m_object.m_model, texture_list->m_star, star->m_z);

			s_point_t point =
			{
				s_random_generate_f(G_GAMESCREEN_SIZE.m_x - 1.0f) - G_GAMESCREEN_SIZE_HALF.m_x,
				s_random_generate_f(G_GAMESCREEN_SIZE.m_y - 1.0f) - G_GAMESCREEN_SIZE_HALF.m_y
			};

			s_model_translate(&star->m_object.m_model, &point);
		}

		lower = upper;
	}
}

void g_star_array_update(g_star_array_t* star_array, const s_point_t* ship_velocity)
{
	if (!ship_velocity)
	{
		return;
	}

	if (s_point_equals_f(ship_velocity, 0.0f, 0.0f))
	{
		return;
	}

	const float speed = s_point_get_length(ship_velocity);
	float angle = atan2f(ship_velocity->m_x, ship_velocity->m_y) + S_PI_HALF;
	const s_point_t velocity = { speed * cosf(angle) * G_STAR_SPEED, speed * sinf(angle) * G_STAR_SPEED };
	s_point_t delta = { 0.0f, 0.0f };

	for (size_t i = 0; i < G_STAR_COUNT; ++i)
	{
		s_point_set(&delta, &velocity);
		s_point_multiply_f(&delta, speed * star_array->m_star[i].m_z);
		delta.m_y = -delta.m_y;

		g_star_update(&star_array->m_star[i], &delta);
	}
}
