...
static void g_models_generate_boulder_model_scaled(g_boulder_model_t* model, float scale, float* max_radius)
{
	...
	for (int32_t i = 0; i < G_BOULDER_TRIANGLE_COUNT; ++i)
	{
		...
		model->m_indices[i * S_TRIANGLE_POINT_COUNT + 1] = i + 2;
		model->m_indices[i * S_TRIANGLE_POINT_COUNT + 2] = i + 1;
	}
	model->m_indices[G_BOULDER_TRIANGLE_COUNT * S_TRIANGLE_POINT_COUNT - 1] = model->m_indices[G_BOULDER_TRIANGLE_COUNT * S_TRIANGLE_POINT_COUNT - 2];
