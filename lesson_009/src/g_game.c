...
void g_game_zero_initialize_data(g_game_data_t* data)
{
	...
	g_star_array_zero_initialize_data(&s_game_data->m_star_array);
...
int32_t g_game_initialize_data()
{
	...
	if (g_boulder_array_initialize(&s_game_data->m_boulder_array, G_BOULDER_INITIAL_COUNT) < 0)
	{
		return -1;
	}
	...
	g_boulder_array_scatter(s_game_data->m_textures.m_boulder, &s_game_data->m_boulder_array);
	...
	s_game_data->m_stats.m_boulder_count = s_vector_get_size(s_game_data->m_boulder_array.m_boulder_list);
	s_game_data->m_stats.m_boulder_capacity = s_vector_get_capacity(s_game_data->m_boulder_array.m_boulder_list);
...
{
	g_boulder_array_free(&s_game_data->m_boulder_array);
...
void g_game_logic()
{
	...
	g_boulder_array_update(&s_game_data->m_boulder_array);
}
...
void g_game_draw()
{
	...
	g_boulder_array_draw(&s_game_data->m_boulder_array, s_game_data->m_draw_flag);
