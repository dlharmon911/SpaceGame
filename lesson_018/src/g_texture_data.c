...
static int32_t g_load_texture_from_archive(g_texture_data_t* textures)
{
	for (size_t i = 0; i < G_TEXTURE_COUNT; ++i)
	{
...
static int32_t g_save_textures(const g_texture_data_t* textures)
{
	for (size_t i = 0; i < G_TEXTURE_COUNT; ++i)
	{
...
int32_t g_texture_data_generate(g_texture_data_t* textures)
{
	int32_t(*function_ptr[])(ALLEGRO_BITMAP**) =
	{
		...
		g_texture_generate_radar_texture
	};
