...
void g_star_array_update(g_star_array_t* star_array, const s_point_t* ship_velocity)
{
	...
	for (size_t i = 0; i < G_STAR_COUNT; ++i)
	{
		...
		s_point_set_f(&star->m_object.m_velocity, velocity.m_x * speed * star->m_z, -velocity.m_y * speed * star->m_z);
		g_star_update(star);
	}
}
...
void g_star_array_update_step(g_star_array_t* star_array, const s_point_t* ship_velocity)
{
	...
	for (size_t i = 0; i < G_STAR_COUNT; ++i)
	{
		...
		s_model_validate_screen(&star->m_object.m_model, &G_GAMESCREEN_TOP_LEFT, &G_GAMESCREEN_BOTTOM_RIGHT);
	}
}
