...
static void g_models_generate_ship_model_vertices(g_ship_model_t* model, const s_point_t* center, float w, float h)
{
	...
	for (int32_t v = 0; v < G_SHIP_VERTEX_COUNT; ++v)
	{
		s_point_set(&model->m_vertices[v].m_point, &vertices[v]);
		s_point_subtract(&model->m_vertices[v].m_point, center);
		s_point_rotate(&model->m_vertices[v].m_point, S_PI_HALF);

		model->m_vertices[v].m_uv.m_x = vertices[v].m_x;
		model->m_vertices[v].m_uv.m_y = vertices[v].m_y;

		model->m_vertices[v].m_color.m_red =
			model->m_vertices[v].m_color.m_green =
			model->m_vertices[v].m_color.m_blue =
			model->m_vertices[v].m_color.m_alpha = 1.0f;
	}
...
void g_model_generate_ship_model(g_ship_model_t* ship_model, s_model_t* model, ALLEGRO_BITMAP* texture)
{
	...
	model->m_vertex_buffer.m_buffer = ship_model->m_vertices;
	model->m_outline_buffer.m_count = G_SHIP_OUTLINE_VERTEX_COUNT;
	model->m_outline_buffer.m_buffer = ship_model->m_outline;

	s_point_set_zero(&model->m_center);
	s_model_scale(model, 0.75f);
}
