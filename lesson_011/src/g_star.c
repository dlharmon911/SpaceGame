...
void g_star_initialize(const g_texture_list_t* texture_list, g_star_t* star, float scale)
{
	if (!texture_list)
	{
		return;
	...
	g_model_generate_star_model(&star->m_star_model, &star->m_object.m_model, texture_list->m_star, scale);
	star->m_z = scale;
}
