...
static void g_models_generate_boulder_model_scaled(g_boulder_model_t* model, float scale)
{
	...
	{
		radius = g_bitmaps_permutate_radius(w * ((G_BOULDER_RADIUS_PERMUTATION_MAX + G_BOULDER_RADIUS_PERMUTATION_MIN) * 0.5f), w * G_BOULDER_RADIUS_PERMUTATION_MIN, w * G_BOULDER_RADIUS_PERMUTATION_MAX);
...
void g_model_generate_boulder_model(g_boulder_model_t* boulder_model, s_model_t* model, ALLEGRO_BITMAP* texture, float scale)
{
	...
	g_models_generate_boulder_model_scaled(boulder_model, scale);
