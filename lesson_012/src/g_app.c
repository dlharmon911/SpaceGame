...
int32_t g_app_initialize(const char* exe_filename, g_app_data_t* data)
{
	...
	s_point_set_f(&data->m_display_scale,
		data->m_settings.m_display.m_size.m_x,
		data->m_settings.m_display.m_size.m_y);
	s_point_multiply(&data->m_display_scale, &G_GAMESCREEN_SCALE);
...
void g_app_loop(g_app_data_t* data)
{
	...
	while (data->m_game_data.m_is_running)
	{
		...
		data->m_game_data.m_stats.m_fps = (1.0 / (end_time - start_time));
		start_time = end_time;
	}
