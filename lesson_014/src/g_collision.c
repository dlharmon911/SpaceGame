...
static bool g_collison_do_boulder_array_vs_bullet_collision(g_boulder_array_t* boulder_array, size_t max_boulder, const g_bullet_t* bullet, g_stats_collision_t* stats)
{
	...
	for (size_t index = 0; index < max_boulder; ++index)
	{
		boulder_node = (g_boulder_node_t*)s_vector_get_value(boulder_array->m_boulder_list, index);
...
bool g_collison_do_boulder_array_vs_bullet_array_collision(g_boulder_array_t* boulder_array, g_bullet_array_t* bullet_array, g_stats_collision_t* stats)
{
	...
	max_boulder = g_boulder_array_get_size(boulder_array);
	...
	for (size_t index = 0; index < max_boulder; ++index)
	{
		...
		if ((bullet_node->m_status & G_BULLET_STATUS_LIVE) &&
			g_collison_do_boulder_array_vs_bullet_collision(boulder_array, max_boulder, &bullet_node->m_bullet, stats))
		{
			s_flags_remove(&bullet_node->m_status, G_BULLET_STATUS_LIVE);
