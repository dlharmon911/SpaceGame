...
{
	g_boulder_node_t boulder_node = { 0 };
	...
	{
		s_vector_push_back(boulder_array->m_boulder_list, &boulder_node);
...
g_boulder_node_t* g_boulder_array_find_first_dead_boulder(g_boulder_array_t* boulder_array)
{
	...
	for (size_t i = 0; i < size; ++i)
	{
		boulder_node = s_vector_get_value(boulder_array->m_boulder_list, i);
		if (!(boulder_node->m_status & G_BOULDER_STATUS_LIVE))
		{
			return boulder_node;
		}
...
void g_boulder_array_scatter(ALLEGRO_BITMAP* texture, g_boulder_array_t* boulder_array)
{
	...
	{
		boulder_node = g_boulder_array_find_first_dead_boulder(boulder_array);
