...
void g_ship_move(g_ship_t* ship, const s_point_t* mouse, int32_t direction)
{
	...
	angle = atan2f(-mouse->m_y, mouse->m_x);
	angle_diff = angle - ship->m_object.m_facing;
	angle_sign = 1.0f;
	...
	if (!s_math_is_zero_f(angle_diff))
	{
		angle_diff = angle_sign * s_math_min_f(fabsf(angle_diff), G_SHIP_ROTATE_AMOUNT);
	}

	if (direction == G_SHIP_MOVE_FORWARD)
	{
		g_ship_thrust(ship, G_SHIP_THRUST);
		g_ship_rotate(ship, angle_diff);
	}
	else
	{
		g_ship_thrust(ship, -G_SHIP_THRUST * G_SHIP_REVERSE_THRUST_RATIO);
		g_ship_rotate(ship, angle_diff * G_SHIP_REVERSE_THRUST_RATIO);
	}
}
